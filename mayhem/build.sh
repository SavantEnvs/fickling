...
#    - astunparse: test_crashes.py imports it directly (this vintage of the suite predates
#      relying solely on ast.unparse).
PYPI_PKGS=(atheris pytest pytest-cov "coverage[toml]" "py7zr>=1.1.0,!=1.1.2" "numpy>=2.3.5" astunparse)
TORCH_PKGS=("torch>=2.1.0" "torchvision>=0.24.1")
need_download=0
"$PY" -c "import os,glob,sys; sys.exit(0 if (glob.glob(os.path.join('$WHEELHOUSE','atheris-*.whl')) and glob.glob(os.path.join('$WHEELHOUSE','torch-*.whl')) and glob.glob(os.path.join('$WHEELHOUSE','astunparse-*.whl'))) else 1)" || need_download=1
...
if "$PY" -c "import os,glob,sys; sys.exit(0 if (glob.glob(os.path.join('$SITE','atheris*')) and glob.glob(os.path.join('$SITE','pytest')) and glob.glob(os.path.join('$SITE','torch')) and glob.glob(os.path.join('$SITE','astunparse*'))) else 1)"; then
...
# A minimal, standalone `distutils.sysconfig` shim: fickling/pickle.py at this vintage does
# `import distutils.sysconfig as sysconfig; sysconfig.get_python_lib(standard_lib=True)`, and
# CPython 3.12+ dropped distutils from the stdlib entirely. setuptools vendors a full distutils
# replacement, but importing it drags in setuptools' entire modern package init (config parsing,
# packaging, importlib.metadata, ...) — atheris then instruments all of it at harness startup,
# slow enough that Mayhem's libFuzzer smoke probe (`-help=1`) timed out waiting for output
# (confirmed: first cloud dispatch failed sanity with "libFuzzer target timed out when printing
# options"). The one call site only needs the stdlib path, which `sysconfig.get_path` still
# provides directly — write just that, no setuptools involved.
mkdir -p "$SITE/distutils"
cat > "$SITE/distutils/__init__.py" <<'EOF'
EOF
cat > "$SITE/distutils/sysconfig.py" <<'EOF'
import sysconfig as _sysconfig


def get_python_lib(plat_specific=0, standard_lib=0, prefix=None):
    return _sysconfig.get_path("platstdlib" if standard_lib and plat_specific
                                else "stdlib" if standard_lib
                                else "platlib" if plat_specific
                                else "purelib")
EOF
...
PYTHONPATH="$PYRUN" "$PY" -c 'import atheris, pytest, torch, torchvision, py7zr, numpy, astunparse, distutils.sysconfig, fickling; print("imports OK: fickling", fickling.__version__, "| torch", torch.__version__)'
